...
- プロンプトキャッシュ: システムプロンプトを不変部（役割・ガイド・プロジェクト情報・ナレッジ）と可変部（現在のフェーズ・整理済みの内容）に分け、不変部と対話履歴の前半にだけ区切りを置く（保持1時間）。1本にまとめると読み出しが当たらず書き込みの割増だけを払うことになる。検査: `check:asyncturn`
...
- 2026-08-30 v1.6 — プロンプトキャッシュの設計見直し（送る内容・モデル・出力は変えずに入力コストを削減）
